...
先采集用户确认的 Zotero 数据目录和读取边界。第一步只处理用户导出的 BibTeX/RIS/CSL JSON 或用户指定的 PDF 文件夹；若重复任务证明需要实时库检索，再用 `scripts/new-artifact.ps1 -Type mcp-adoption-review -Name "zotero-readonly"` 生成 MCP 评估记录。
