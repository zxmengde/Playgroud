...
本轮已新增 `docs/assistant/security-model.md` 和 `scripts/scan-text-risk.ps1`，并让系统校验调用扫描脚本。该脚本不能替代人工安全审查，但可以拦截一类常见不可见注入载体。
...
- 在后续第三方 skill 审查中使用 `scripts/scan-text-risk.ps1`。
