...
当前控制仓库主要用于个人工作系统维护，优先保持提交历史可审计。若后续创建 PR，默认使用草稿 PR，合并前运行 `scripts/lib/commands/check-finish-readiness.ps1`。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "$env:CODEX_WORKTREE_PATH\scripts\lib\commands\setup-codex-environment.ps1" -RunValidation
```
