...
- 权限模型：哪些动作可自动执行，哪些动作需要任务级授权或预授权，是否存在一次授权后长期放权。
...
Cline 与 Roo Code 的模式、规则和 auto-approve 资料说明，长任务需要减少无意义确认，但不能取消可审计边界。对本系统的启发是：读取、检查、生成草稿、运行测试和可信仓库内可回退操作可默认执行；删除、覆盖、外部写入和账号操作应进入任务级授权或预授权。

## Windows 与 Office 自动化启发

Power Automate Desktop 适合稳定、重复、界面明确的 Windows 流程。无人值守流程涉及账号、许可、会话状态和审计，应仅在规则明确、失败可发现、存在任务级授权或预授权后配置。
...
Playwright 与 Chrome DevTools MCP 适合网页访问、截图、控制台检查和前端验证。涉及登录、表单、下载敏感资料或外部写入时，需要任务级授权或预授权。
...
第三方 skills 默认只读评估。除非来源可信、权限清楚、脚本可审计、验证通过且存在任务级授权或预授权，否则不安装。围绕泄露源码传播的仓库、压缩包和安装脚本不得下载或复制。
