...
learned_mechanism: raw note、frontmatter、vault targeting、Obsidian Markdown 和仓库 knowledge 的边界应分开。  
local_artifact: `docs/workflows/knowledge.md`、`docs/core/typed-object-registry.md`、`docs/capabilities/capability-map.yaml`。  
trigger_condition: 信息需要从临时记录提升为长期 knowledge，或用户明确要求写 Obsidian。  
codex_behavior_delta: 默认 repository knowledge-first；只有目标 vault、路径、回退方式明确时才写外部 Obsidian；避免把一次性信息写入长期 memory。  
user_visible_entry: `scripts/codex.ps1 knowledge check`、`scripts/codex.ps1 knowledge obsidian-dry-run`。  
verification: knowledge workflow 写明 raw note -> curated note -> verified knowledge -> archived/superseded；Obsidian 仍按 dry-run 或任务授权执行。  
rollback: 删除 knowledge lifecycle 更新，保留原 `knowledge obsidian-dry-run`。
partial_notes: 未吸收 JSON Canvas、批量 vault patch 和完整 Obsidian skill pack。
...
local_artifact: `docs/tasks/board.md`、`docs/tasks/active.md`、`scripts/codex.ps1 task recover`。  
trigger_condition: 多任务、长期任务、中断恢复、blocked 状态或 next action 不清。  
codex_behavior_delta: 不再只靠单个 active.md；任务状态必须包含 active/next/blocked/done、checkpoint、resume summary、stale detection 和 next action。  
user_visible_entry: `docs/tasks/board.md`、`scripts/codex.ps1 task recover`。  
verification: `validate-delivery-system.ps1` 检查 board 字段；本轮 active task 被重写。  
rollback: 删除 `board.md` 并恢复旧 active/done 摘要。
partial_notes: 未吸收 server、workspace UI、task attempt 数据库和 Git 操作服务。
...
local_artifact: `docs/knowledge/research/research-queue.md`、`docs/validation/real-task-evals.md`、`docs/core/tool-use-budget.md`。  
...
user_visible_entry: `docs/knowledge/research/research-queue.md`、`scripts/codex.ps1 research run-log`。  
verification: `validate-delivery-system.ps1` 检查 research queue 关键字段；capability maturity 只能是 experimental。  
rollback: 删除 research queue 文件并保留原 research run log。
partial_notes: 未吸收 watchdog、后台守护进程、通知服务和无人值守执行 runtime。
