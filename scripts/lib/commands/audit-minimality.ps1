...
$legacyCapFiles = @($capFiles | Where-Object { $allowedCapabilityFiles -notcontains $_.Name })
...
$topLevelScriptFiles = @(Get-ChildItem -Path (Join-Path $Root "scripts") -Filter "*.ps1" -File -ErrorAction SilentlyContinue)
$allScriptFiles = @(Get-ChildItem -Path (Join-Path $Root "scripts") -Recurse -Filter "*.ps1" -File -ErrorAction SilentlyContinue)
...
if ($topLevelScriptFiles.Count -gt 5) { $failures += "Too many top-level PowerShell scripts remain." }
...
Write-Output ("top-level PowerShell scripts: {0}" -f $topLevelScriptFiles.Count)
Write-Output ("total PowerShell scripts: {0}" -f $allScriptFiles.Count)
