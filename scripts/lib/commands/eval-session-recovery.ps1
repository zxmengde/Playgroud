...
$alwaysBlock = ""
$alwaysMatch = [regex]::Match($context, '(?ms)^Always load:\s*\r?\n(.*?)(?=^\s*$)', [System.Text.RegularExpressions.RegexOptions]::Multiline)
if ($alwaysMatch.Success) {
    $alwaysBlock = $alwaysMatch.Groups[1].Value
}
if ($alwaysBlock -like "*2026-04-27-codex-self-improvement-report.md*") {
