...
$serenaBlockMatch = [regex]::Match($config, '(?ms)^\[mcp_servers\.serena\]\s*(.*?)(?=^\[|\z)')
if (-not $serenaBlockMatch.Success) {
    throw "Codex config does not contain a Serena MCP entry."
}
$serenaBlock = $serenaBlockMatch.Groups[1].Value
if ($serenaBlock -notmatch 'command\s*=\s*"(serena|uvx)"') {
    throw "Serena MCP entry must use command serena or uvx."
}
if ($serenaBlock -notmatch 'start-mcp-server') {
    throw "Serena MCP entry does not start the MCP server."
}
