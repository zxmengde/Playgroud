---
name: aris-skills-codex-grant-proposal
...
### Merged Sources
- `aris-grant-proposal`: 655 lines, sha `9e9cd5c420bfd66a`, source-overlap `0.91`. Trigger: Draft a structured grant proposal from research ideas and literature. Supports KAKENHI (Japan), NSF (US), NSFC (China, including 面上/青年/优青/杰青/海外优青/重点), ERC (EU), DFG (Germany), SNSF (Switzerland), ARC (Australia), NWO (Netherlands), and generic formats. Use whe
- `aris-skills-codex-gemini-review-grant-proposal`: 628 lines, sha `f8fbcdd83e144453`, source-overlap `0.95`. Trigger: Draft a structured grant proposal from research ideas and literature. Supports KAKENHI (Japan), NSF (US), NSFC (China, including 面上/青年/优青/杰青/海外优青/重点), ERC (EU), DFG (Germany), SNSF (Switzerland), ARC (Australia), NWO (Netherlands), and generic formats. Use whe
...
- Source-specific retained points from `aris-grant-proposal`:
  - (survey) (verify gap) (aims + matrix) (prose) (panel review) (fix) (done!)
  - ┌→ /experiment-bridge → /auto-review-loop → /paper-writing (publish track)
  - └→ /grant-proposal → [get funded] → /experiment-bridge → ... (funding track)
  - **REVIEWER_MODEL = `gpt-5.4`** — Model used via Codex MCP for proposal review. Must be an OpenAI model (e.g., `gpt-5.4`, `o3`, `gpt-4o`).
- Source-specific retained points from `aris-skills-codex-gemini-review-grant-proposal`:
