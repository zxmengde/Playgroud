...
## Consolidated Security Review Merge

This keeper replaces `omx-security-review` and `omx-plugin-security-review`.
Their source copies remain under `references/imported-variants-2026-05-04/` for
rollback/deep reference only.

### Expanded Trigger Coverage

Use this skill for explicit security review/audit requests, secure-by-default
implementation, authentication or authorization changes, new API endpoints,
untrusted input handling, file upload, cryptography, dependency additions,
production deployment review, and suspected secret exposure.

### Security Review Workflow

1. Scope the review: changed files, exposed routes, trust boundaries,
   authentication, authorization, user input, output encoding, secrets,
   cryptography, dependencies, storage, logging, and deployment surface.
2. Identify languages and frameworks, then load only matching reference files
   from this skill. For web apps, check frontend and backend guidance.
3. Form an independent local analysis before using any external reviewer or
   tool. External review can validate findings but cannot replace code evidence.
4. Check OWASP Top 10 classes: broken access control, cryptographic failures,
   injection, insecure design, misconfiguration, vulnerable components,
   authentication failures, integrity failures, logging/monitoring gaps, and SSRF.
5. Check repository-specific risks: hardcoded secrets, weak tokens or sessions,
   unsafe dependency additions, file/path traversal, unescaped output, command
   execution, unsafe deserialization, missing authorization before action, and
   sensitive data in logs or errors.
6. If making fixes, address one finding at a time, preserve behavior, run
   relevant regression tests, and re-check the specific security issue.

### Severity Contract

- `CRITICAL`: exposed secret, remote code execution, authentication bypass,
  direct data breach, exploitable injection, or severe credential exposure.
- `HIGH`: serious authorization/data-integrity weakness, weak password or token
  handling, high-impact dependency CVE, or exploitable XSS/SSRF/path traversal.
- `MEDIUM`: incomplete validation, weak defense-in-depth, excessive error
  detail, insufficient rate limiting, or security logging gap.
- `LOW`: hardening suggestion or best-practice improvement with limited impact.

### Finding Contract

Each finding must include file/line evidence where available, affected trust
boundary, plausible exploit or impact path, remediation, verification method,
and relevant standard or CVE when useful. Do not report lack of TLS, HSTS, or
secure cookies as a blanket issue in local development without deployment
evidence, because those fixes can break non-TLS dev/test setups.

### Output Contract

For a report, write or return a severity-grouped markdown report with executive
summary, findings, remediation priority, dependency vulnerability summary when
checked, and final deployment recommendation. If a secret is found, state that
rotation is required; code removal alone is not enough.
