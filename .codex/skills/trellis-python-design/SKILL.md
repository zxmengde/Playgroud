...
Replaces the platform-specific `python-design` duplicates.

### Retained Rules
- Use for Python CLI/scripts, module decomposition, refactoring, and review.
- Optimize for complexity reduction: reduce change amplification, cognitive load, and unknown unknowns.
- Prefer deep modules with simple interfaces hiding path, JSON, parsing, defaults, and error handling.
- Define data shapes and function signatures first with dataclasses, TypedDict, NewType, and discriminated unions where useful.
- Validate at boundaries, avoid scattered `.get()` defaults, pass-through wrappers, and shallow helpers that leak internals.
