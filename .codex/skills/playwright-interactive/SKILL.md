...
description: "Use for persistent Playwright browser or Electron sessions through `js_repl` when UI debugging needs reused handles, iterative screenshots, mobile/desktop passes, or visual QA without restarting the browser. Do not trigger for one-shot terminal CLI automation."
metadata:
