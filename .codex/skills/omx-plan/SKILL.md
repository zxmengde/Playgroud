...
Replaces `omx-plugin-plan`.

### Retained Rules
- Use for planning when scope, risks, acceptance criteria, or implementation order are not yet stable.
- Produce a concrete plan artifact with goals, non-goals, constraints, risk surface, acceptance tests, and next action.
- In review mode, keep writer/reviewer separation and return an approval/revision verdict with evidence.
- Do not keep planning when the next safe implementation step is already clear.
