---
name: omx-configure-notifications
...
Replaces `omx-plugin-configure-notifications`.

### Retained Rules
- Use only when the user asks to configure notification behavior or delivery channels.
- Separate channel setup, credential requirements, test notification, and rollback.
- Do not store tokens or external account secrets without explicit authorization.
- Verify with a harmless test event when possible and report the exact config path touched.
