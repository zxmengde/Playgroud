...
> Compatibility: this project skill was shortened from `trellis-packages-cli-src-templates-codex-skills-update-spec`; route old references here.
...
## Merged Trellis Spec Capture Rules

Apply these rules when implementation, debugging, or design decisions reveal durable project knowledge. They are synthesized from non-Codex Trellis variants.

- Capture executable contracts, signatures, validation matrices, and test points for infra or cross-layer changes.
- Promote lessons only when they prevent repeated mistakes or clarify future implementation, not for one-off task notes.
- Preserve code examples as templates when the pattern is likely to recur.
- Update indexes or discovery files so future sessions can find the captured knowledge.
- Read references/imported-variants-2026-05-04/synthesized-insights.md before deciding whether a debugging or implementation lesson belongs in specs.

## Consolidated Trellis Skill Merge

Replaces the platform-specific `trellis-update-spec` duplicates.

### Retained Rules
- Use code-spec depth for executable contracts, not principle-only notes.
- Mandatory triggers: command/API signature changes, cross-layer contract changes, database schema/migration, infra integration, secrets/env wiring, or validation/error behavior.
- Include seven sections when triggered: scope/trigger, signatures, contracts, validation/error matrix, good/base/bad cases, tests required, wrong-vs-correct.
- Distinguish code-specs from guides: implementation contracts belong in layer specs; thinking prompts belong in `guides/`.
- Read the target spec before editing, avoid duplication, update indexes when needed, and keep one concept per section.
