...
Replaces `omx-plugin-ultrawork`.

### Retained Rules
- Use for high-throughput execution only after task intent, constraints, and pass/fail criteria are grounded.
- Parallelize only independent work; keep shared-file or tightly coupled implementation local.
- Ultrawork provides execution and lightweight evidence; Ralph adds persistence and verified-completion guarantees.
- Final claim must stay within the evidence actually produced by ultrawork.
