...
> Compatibility: this project skill was shortened from `trellis-packages-cli-src-templates-codex-skills-onboard`; route old references here.
...
Replaces the archived Trellis bootstrap skill.

### Retained Rules
- Use onboarding when a developer needs to understand or initialize Trellis workflow concepts.
- Explain why AI memory, project-specific specs, and context-drift checks exist, not just command names.
- Cover system structure, workflow examples, and customization of `.trellis/spec/` guidelines.
- For bootstrap, verify Node/npm, install/check Trellis CLI, run `trellis init -u <developer>`, and verify `.trellis/`, platform commands, and `AGENTS.md` exist.
- Treat bootstrap as one-time setup; after initialization, use start/before-dev/check/finish-work workflows.
