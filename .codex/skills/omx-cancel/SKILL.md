...
Replaces `omx-plugin-cancel`.

### Retained Rules
- Use to stop or roll back an active OMX workflow, not as a generic refusal mechanism.
- Identify the active state root, workflow, task/worker/session IDs, and current lifecycle status before cancelling.
- Prefer graceful cancellation: mark state, release claims, notify workers, and preserve logs/artifacts.
- If forced cleanup is required, report what state was changed and what remains for manual recovery.
