...
Replaces platform/common `trellis-meta` duplicates.

### Retained Rules
- Treat the user's local project files as authoritative: `.trellis/`, platform directories, shared `.agents/skills/`, hooks, commands, prompts, and settings.
- Do not modify global npm installs or `node_modules/@mindfoldhq/trellis` for local project needs.
- Before customization, read local architecture, platform map, customization overview, and the actual target files.
- Keep project-private conventions in `.trellis/spec/` or project-local skills, not in global/public Trellis meta rules.
- Preserve generated-file boundaries and do not overwrite user-modified local files with default templates.
