...
Replaces `omx-plugin-worker`.

### Retained Rules
- Use only inside an OMX Team worker session with `OMX_TEAM_WORKER` set.
- Send startup ACK to the leader mailbox before task work.
- Read inbox, claim one unblocked task via CLI/state API, complete or fail it through lifecycle transition, then update worker status.
- Do not mutate task lifecycle fields directly; use the claim-safe CLI interop.
- Treat manual terminal nudges only as a prompt to re-check durable state.
