...
Replaces `omx-plugin-pipeline`.

### Retained Rules
- Use when the user wants a staged workflow with explicit phase handoffs.
- Each phase must have input, output, owner, verification, and stop condition.
- Preserve artifacts between phases so resume does not depend on conversation memory.
