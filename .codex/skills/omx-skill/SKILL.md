...
Replaces `omx-plugin-skill`.

### Retained Rules
- Manage skills across user and project scopes; always check both before add/remove/search/edit.
- Validate skill frontmatter and naming after changes.
- For new skills, preserve concise trigger metadata and move long examples/references out of the main `SKILL.md`.
- Do not delete skills irreversibly; move to a dated disabled/backup location unless the user explicitly authorizes deletion.
