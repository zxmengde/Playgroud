...
Replaces `omx-plugin-autoresearch`.

### Retained Rules
- Use for research queue or autonomous research loops only when the user asks for research continuation rather than ordinary web lookup.
- Record the research question, source, evidence requirements, review gate, run log, and next action before treating work as resumable.
- Do not claim background service behavior unless an actual service/watchdog is running and reported.
- Research output must distinguish leads, verified evidence, claims, limitations, and follow-up experiments or reading.
