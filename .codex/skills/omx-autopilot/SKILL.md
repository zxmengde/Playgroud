...
Replaces `omx-plugin-autopilot`.

### Retained Rules
- Use only for hands-off non-trivial delivery where the user wants planning, implementation, verification, and review to loop.
- Primary contract is `ralplan -> ralph -> code-review`; do not invent a separate lifecycle around it.
- If code review is not clean, return findings to `ralplan`, update the plan/test spec, rerun `ralph`, then rerun review.
- A clean exit requires code-review `APPROVE` and architecture `CLEAR`; `WATCH`, `BLOCK`, `COMMENT`, or unresolved findings are not clean.
- Persist cycle state, review verdict, return reason, and hard blockers so resume does not restart from vague context.
