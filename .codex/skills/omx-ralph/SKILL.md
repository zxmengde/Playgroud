...
Replaces `omx-plugin-ralph`.

### Retained Rules
- Use for persistent implementation from an approved plan or clear task target.
- Ralph owns edits, targeted verification, changed-file summary, deslop where applicable, and handoff to review.
- Keep state/artifacts sufficient for resume: task, plan, attempts, verification, blockers, and next action.
- Do not claim verified completion without tests/build or an explicit reason verification could not run.
