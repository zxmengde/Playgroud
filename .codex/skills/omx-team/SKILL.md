---
name: omx-team
description: N coordinated agents on shared task list using tmux-based orchestration. Also owns the old swarm compatibility route; route swarm requests here instead of loading a separate facade skill.
metadata:
...
`$swarm` is only a compatibility alias. Route swarm requests to this skill and
invoke the same `omx team ...` lifecycle; do not keep a separate swarm facade.
...
Replaces `omx-plugin-team`.

### Retained Rules
- Use only when the durable tmux/OMX team runtime is active or explicitly requested.
- Define leader, workers, team state root, task queue, mailbox protocol, and lifecycle before dispatch.
- Worker tasks must be claim-safe and transition through pending/in_progress/completed or failed using CLI/state API.
- Prefer durable inbox/mailbox/task files over ad hoc terminal keystrokes.
