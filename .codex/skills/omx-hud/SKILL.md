...
Replaces `omx-plugin-hud`.

### Retained Rules
- Use for compact current-state display: active workflow, task, workers, progress, blockers, and next action.
- Prefer reading durable OMX state over inferring from conversation.
- Do not mutate state from HUD.
