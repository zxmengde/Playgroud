...
> Compatibility: this project skill was shortened from `trellis-packages-cli-src-templates-codex-skills-brainstorm`; route old references here.
...
## Merged Trellis Brainstorm Rules

Apply these rules when requirements are unclear or several implementation paths are possible. They are synthesized from non-Codex Trellis variants.

- Act before asking low-value questions: inspect existing configs, scripts, routes, commands, and conventions first.
- Use research-first option generation for technical choices. Present concrete options with tradeoffs before asking the user to choose.
- Ask one high-value question at a time only after narrowing uncertainty with local evidence.
- Keep acceptance criteria as testable checkboxes and update them after each user decision.
- If parallel research is available and useful, delegate isolated research without polluting the brainstorm context with raw output.
- Read references/imported-variants-2026-05-04/synthesized-insights.md for the imported Trellis research prompts and anti-patterns.

## Consolidated Trellis Skill Merge

Replaces the platform-specific `trellis-brainstorm` duplicates.

### Retained Rules
- Ensure a task/PRD artifact exists before extended Q&A so ideas are captured immediately.
- Inspect repo code, docs, configs, conventions, and quick technical evidence before asking the user questions.
- Ask one high-value blocking or preference question at a time; do not ask meta questions when local inspection can answer.
- Use research-first mode for technical choices, then present 2-3 concrete options with tradeoffs.
- Expand to edge cases and future evolution, then converge to MVP scope, acceptance criteria, and explicit out-of-scope.
