...
> Compatibility: this project skill was shortened from `trellis-packages-cli-src-templates-codex-skills-before-dev`; route old references here.
...
Replaces the platform-specific `trellis-before-dev` duplicates.

### Retained Rules
- Before coding, discover applicable `.trellis/spec/` package and layer context with the project script when available.
- Read the relevant spec index, then the actual guideline files listed by the pre-development checklist; the index is a pointer, not the goal.
- Always read shared guides when present.
- Use this step when starting a coding task, switching packages, or refreshing project conventions before implementation.
